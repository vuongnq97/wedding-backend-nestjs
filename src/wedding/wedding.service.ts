    ...
    async create(dto: WeddingDto, userId: string): Promise<WeddingDto> {
        ...
        const created = await this.prisma.wedding.create({
            data: {
                ...
                notification: (dto.notification || { line1: '', line2: '' }) as any,
                ceremony: (dto.ceremony || { show: false }) as any,
                reception: (dto.reception || {}) as any,
                map: (dto.map || { show: false }) as any,
                music: (dto.music || { enabled: false }) as any,
                bankAccounts: (dto.bankAccounts || []) as any,

                thankYouMessage: dto.thankYouMessage,
                guestbookEnabled: dto.guestbookEnabled ?? true,
                showAds: dto.showAds ?? false,
                showLoveStory: dto.showLoveStory ?? true,
            },
        });
